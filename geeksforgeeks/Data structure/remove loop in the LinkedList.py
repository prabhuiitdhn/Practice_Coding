"""
problem: https://practice.geeksforgeeks.org/problems/remove-loop-in-linked-list/1?page=1&company[]=Adobe&company[]=Samsung&company[]=Qualcomm&company[]=KLA%20Tencor&category[]=Linked%20List&sortBy=difficulty
problem reference:https://practice.geeksforgeeks.org/problems/detect-loop-in-linked-list/1?page=1&company[]=Samsung&company[]=Qualcomm&company[]=KLA%20Tencor&category[]=Linked%20List&sortBy=difficulty
Problem is to find the loop in linkedlist and remove the loop. so, if self.tail[Last node] of the list is connected with any node which is already visited then It detects the loop.
...
Approach for removing the loop:
detect the loop in the linkedList and refer the next pointer of current is None. which will detach the loop
"""
...
class LinkedList:
    ...
    def removeLoop(self):
        ...
        while current.next:
            ...
            # check If next is also -1 i.e. visited the detach the connection
            if current.next.data == -1:
                current.next = None
                current = current.next
                break # break meaning. Lop should break here.
            current = current.next
